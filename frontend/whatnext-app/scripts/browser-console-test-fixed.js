...
function handleRecommendations(response) {
  ...
  if (response.recommendations && response.recommendations.length > 0) {
    ...
    console.log('\n%c📽️ First Movie:', 'color: cyan; font-weight: bold');
    console.table({
    ...
    if (window.location.pathname === '/recommendations') {
      console.log('\n%c📍 On recommendations page', 'color: cyan');
      
      setTimeout(() => {
        // Look for any rendered content
        const allElements = document.querySelectorAll('*');
        const movieRelated = Array.from(allElements).filter(el => {
          const text = el.textContent || '';
          const className = el.className || '';
          return text.includes(firstMovie.title) || 
                 className.toLowerCase().includes('movie') ||
                 className.toLowerCase().includes('card');
        });
        
        console.log(`Found ${movieRelated.length} movie-related elements`);
        
        if (movieRelated.length === 0) {
          console.log('%c❌ Movies not rendering on page', 'color: red; font-weight: bold');
          console.log('%c💡 The recommendations page is not displaying the data', 'color: yellow');
        } else {
          console.log('%c✅ Movie elements found on page', 'color: #00ff00');
        }
      }, 1000);
    }
    ...
    console.log('localStorage: debug_recommendations, debug_sessionId');
    console.log('sessionStorage: recommendations_data, session_id');
