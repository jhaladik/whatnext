  ...
  async searchMovies(queryOrVector, filters = {}, limit = 50) {
    try {
      ...
      if (isTextQuery && (this.vectorizeService || this.vectorizeWorkerUrl)) {
        ...
        console.log('[VectorSearch] Received from Vectorize worker:', {
          resultCount: data.results?.length || 0,
          firstResult: data.results?.[0] ? {
            title: data.results[0].title,
            has_poster_path: !!data.results[0].poster_path,
            poster_path: data.results[0].poster_path
          } : null
        });
        
        // Map the results to expected format (tmdb_id -> movieId) with ALL fields
        processedResults = (data.results || []).map(movie => ({
          ...
          // Add comprehensive fields from Vectorize worker
        ...
        // Note: The Vectorize worker now returns poster_path directly
        // No need to query D1 since the movie_cache table is empty
        // The MovieEnrichmentService will convert poster_path to full URLs
        console.log('[VectorSearch] Using poster_path from Vectorize worker directly');
