...
export class MovieEnrichmentService {
  ...
  async enrichMovieResults(movieResults) {
    ...
    const enrichedResults = movieResults.map(movie => {
      // Log the raw movie data to debug
      console.log(`[MovieEnrichment] Processing movie:`, {
        title: movie.title,
      ...
      const enrichedMovie = {
        ...
        // Use poster if already enriched from D1, otherwise build from poster_path
        poster: movie.poster || (posterPath ? `https://image.tmdb.org/t/p/w500${posterPath}` : null),
        backdrop: movie.backdrop || (movie.backdrop_path ? `https://image.tmdb.org/t/p/w1280${movie.backdrop_path}` : null),
        releaseDate: movie.release_date || movie.releaseDate,
        // Add all the new comprehensive fields
      ...
      console.log(`[MovieEnrichment] Generated poster URL:`, enrichedMovie.poster);
      return enrichedMovie;
    });
    
    console.log(`[MovieEnrichment] Enriched ${enrichedResults.length} movies`);
    if (enrichedResults.length > 0) {
      console.log(`[MovieEnrichment] First movie poster:`, enrichedResults[0].poster);
    }
